...
import { shouldSkipAI } from './ai/modelPipelines';
...
export const parseCreditReport = async (text: string, useAIFirst = true): Promise<CreditReport> => {
  ...
  try {
    ...
    // Check if AI should be skipped due to connection issues
    const skipAI = shouldSkipAI();
    
    // If AI-first approach is enabled and not skipped, try that first
    if (useAIFirst && !skipAI) {
      try {
        ...
        // If AI parsing returned results
        if (aiResults) {
      ...
      console.log("Using simpler parsing approach due to AI being disabled");
    }
    ...
    // Only try to enhance with AI if it's not disabled
    if (!skipAI) {
      try {
      ...
      // Skip AI enhancement
      console.log("Skipping AI enhancement due to previous errors");
      parsingLogger.completeParsing();
      return initialReport;
    }
  } catch (error) {
